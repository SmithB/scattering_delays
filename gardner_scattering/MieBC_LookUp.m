function [qext0, qsca0, g0] = MieBC_LookUp(r0, WL0)
%% Mie qext qsca g look-up function for black carbon
...
%% CHECK IF LOOK-UP VALUES ARE OUT OF TABLE RANGE
if r0 > max(r) || r0 < min(r)
    error('effective grain radius r is out of MieBC_LookUp range')
elseif nargin == 2 
    if WL0 > max(WL) || WL0 < min(WL)
     error('wavelength WL is out of MieBC_LookUp range')
    end
end
